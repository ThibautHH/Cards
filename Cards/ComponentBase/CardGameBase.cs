...
namespace Cards.ComponentBase
{
    public abstract class CardGameBase : Microsoft.AspNetCore.Components.ComponentBase, IAsyncDisposable
    {
        ...
        [Inject] protected HttpContextService HttpContextService { get; set; } = default!;

        protected abstract string GameName { get; }

        protected Player? CurrentPlayer =>
            this.playerList.players.SingleOrDefault(player => player.Name == this.HttpContextService.CurrentUser.Name);
        ...
        protected async void Play() =>
            await this.playerList.hubConnection
               .InvokeAsync("Update", PlayerListAction.Signup, this.HttpContextService.CurrentUser.Name,
                    this.GameName);

        protected async void Quit() =>
            await this.playerList.hubConnection
               .InvokeAsync("Update", PlayerListAction.Quit, this.HttpContextService.CurrentUser.Name,
                    this.GameName);

        protected async void Ready() =>
            await this.playerList.hubConnection
               .InvokeAsync("Update", PlayerListAction.Ready, this.HttpContextService.CurrentUser.Name,
                    this.GameName);

        protected async void Unready() =>
            await this.playerList.hubConnection
               .InvokeAsync("Update", PlayerListAction.Unready, this.HttpContextService.CurrentUser.Name,
                    this.GameName);
    }
}
