    ...
    public readonly struct Card
    {
            ...
            Player? player = null)
        {
            ...
            this.Color = this.Deck != CardDeck.Tarot
                ...
                {
                    CardColor.Trump =>
                        throw new ArgumentException("Cannot set card color to trump" + " if it is not a Tarot card.",
                            nameof(color)),
                    _ => color
                }
                : color;
                ...
                ? height switch
                {
                    (>= CardHeight.Magician) or CardHeight.Knight =>
                        throw new ArgumentException(
                            "Cannot set card height to trump or Knight" + " if it is not a Tarot card.",
                            nameof(height)),
                    _ => height
                }
                : this.Deck == CardDeck.Player32
                    ? height switch
                    {
                        (<= CardHeight.Six) and not CardHeight.Ace =>
                            throw new ArgumentException(
                                "Cannot set card height to lower than seven" + " if it is a 32-cards deck card.",
                                nameof(height)),
                        _ => height
                    }
                    : height;
